...
class _ZdsHtmlContainerState extends State<ZdsHtmlContainer> with FrameCallbackMixin {
  ...
  static final RegExp mediaRegex = RegExp('<figure class="media">.*?data-oembed-url="(.*?)".*?</figure>', dotAll: true);
  static final RegExp loaderRegex = RegExp(r'<loader\s+src=(\S+)></loader>', dotAll: true);
  static final RegExp iFrameRegex = RegExp('<iframe.*?src="(.*?)".*?</iframe>', dotAll: true);
  ...
  void _updateHasMore() {
    ...
    )..layout(maxWidth: containerWidth ?? MediaQuery.of(context).size.width);
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (BuildContext ctx, BoxConstraints box) {
        atLast(() {
          _setContainerWidth(box.maxWidth);
        });
