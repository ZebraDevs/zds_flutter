...
class _ZdsQuillEditorState extends State<ZdsQuillEditorPage> {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Stack(
          children: <Widget>[
            editor,
            if (widget.charLimit != 0 && widget.charLimit != 10000) _buildCharacterCountIndicator(context),
          ],
        ),
      ),
    );
      ...
      actions: <Widget>[
        if (widget.toolbarOptions.contains(QuillToolbarOption.undo))
          _HistoryButton(
            ...
            controller: _quillController,
            enabledColor: themeData.appBarTheme.foregroundColor ?? Colors.white,
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    super.debugFillProperties(properties);
    properties.add(DiagnosticsProperty<bool>('showingHtmlError', showingHtmlError));
  }
}
